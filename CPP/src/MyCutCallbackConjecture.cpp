...
int contadorConjucture = 0;

MyCutCallbackConjecture::MyCutCallbackConjecture(IloEnv env, const IloIntVarArray &x_ref) : IloCplex::UserCutCallbackI(env), x(x_ref), x_vars(env)
//MyCutCallbackConjecture::MyCutCallbackConjecture(IloEnv env, const IloIntVarArray &x_ref) : IloCplex::LazyConstraintCallbackI(env), x(x_ref), x_vars(env)
{
   //preenche x_vars
   /*
   ...
   */
   ...
   std::cout << "Inside Callback Conjecture Constructor" << std::endl;
...
IloCplex::CallbackI *MyCutCallbackConjecture::duplicateCallback() const
{
   return new (getEnv()) MyCutCallbackConjecture(getEnv(), x);
}

std::vector<IloConstraint> *MyCutCallbackConjecture::separate()
{
...
void MyCutCallbackConjecture::main()
{

   //contadorConjucture = contadorConjucture + 1;
   ...
   //std::cout << "contadorConjucture: " << contadorConjucture << std::endl;
}
